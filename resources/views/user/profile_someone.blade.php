    ...
    <div class="panel panel-default">
        <div class="panel-body">
            ...
            <div class="row" style="margin-top: 12px">
                ...
                <div class="col-md-3 col-xs-3">
                    <a role="button" id="button-following" class="btn btn-primary btn-brawijaya btn-block following-btn">{{ $data['following_count'] }} Following</a>
                </div>
                <div class="col-md-3 col-xs-3">
                    <a role="button" id="button-follower" class="btn btn-primary btn-brawijaya btn-block follower-btn">{{ $data['follower_count'] }} Follower{{($data['follower_count'] >= 2)? 's':''}}</a>
                </div>
                <div class="col-md-3 col-xs-3">
                    <a role="button" id="button-follower" href="{{route('user-conversation', ['id' => $data['identity']->id])}}" class="btn btn-primary btn-brawijaya btn-block">Send Message</a>
                </div>
            </div>
            <div class="row" style="margin-top: 10px;">
                <div class="col-md-12 col-xs-12 follow" style="{{ (!$data['is_followed']) ? '' : 'display: none;' }}">
                    <a role="button" id="button-follow"  data-idUser="{{ $data['identity']->id }}" class="btn btn-primary btn-brawijaya btn-follow btn-block">Follow</a>
                </div>
                <div class="col-md-12 col-xs-12 unfollow" style="{{ ($data['is_followed']) ? '' : 'display: none;' }}">
                    <a role="button" id="button-unfollow"  data-idUser="{{ $data['identity']->id }}" class="btn btn-primary btn-brawijaya btn-unfollow btn-block">Unfollow</a>
                </div>
            </div>
        </div>
    </div>
